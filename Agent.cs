    ...
    public override void calculateAIStep()
    {
        ...
        switch (MosqutioAIstate)
        {
            ...
            case 6:
                ...
                //select a random movement target with bias towards current position
                float rndX = GD.Randf() * environment.height;
                float mixX = GD.Randf() * 0.8f;
                float rndY = GD.Randf() * environment.width;
                float mixY = GD.Randf() * 0.8f;
                float targetCellX = rndX * (1 - mixX) + position.X * mixX;
                float targetCellY = rndY * (1 - mixY) + position.Y * mixY;
...
public class Human : Agent
{
    ...
    public override void calculateAIStep()
    {
        if (recovering)
        {
            ...
            {
                recovering = false;
                intents.Add(new RecoverIntent(this));
            }
        }
        else if (infected)
        {
            recoveryTimer = GD.RandRange(8 * 1440, 30 * 1440);
            recovering = true;
        }
    }
